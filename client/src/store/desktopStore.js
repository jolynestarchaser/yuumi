...
export const useDesktopStore = create((set, get) => ({
  ...
  deleteItem: async (id) => { const previous = get().items; set((state) => ({ items: state.items.filter((item) => item._id !== id), windows: state.windows.filter((window) => window.itemId !== id) })); try { await api.delete(`/items/${id}`); get().socket?.emit('desktop:broadcast', { type: 'item:deleted', payload: { id } }); } catch { set({ items: previous }); } },
