...
export interface ChimeContext {
  ...
  createOscillator(): {
    type: string; frequency: { setValueAtTime(value: number, time: number): void };
    connect(target: object): void; start(time: number): void; stop(time: number): void;
  };
  createGain(): {
...
export async function playLetterChime({ enabled = true, AudioContextClass }: { enabled?: boolean; AudioContextClass?: new () => ChimeContext } = {}) {
