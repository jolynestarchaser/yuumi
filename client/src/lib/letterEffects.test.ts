...
import type { ChimeContext } from './letterEffects.js';
...
test('letter chime schedules three notes when audio is available', async () => {
  class FakeAudioContext {
    ...
    currentTime: number;
    destination: object;
    oscillators: ReturnType<ChimeContext['createOscillator']>[];
...
test('letter chime fails silently when audio initialization is blocked', async () => {
  ...
  // This constructor intentionally throws before an audio instance exists.
  assert.equal(await playLetterChime({ AudioContextClass: BlockedAudioContext as new () => ChimeContext }), false);
});
