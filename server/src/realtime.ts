...
export function setupRealtime(io) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('item:lock', ({ id }, ack = (_result: Record<string, unknown>) => {}) => {
    ...
    socket.on('item:commit', async ({ id, parentId = null, position, revision }, ack = (_result: Record<string, unknown>) => {}) => {
    ...
    socket.on('ink:commit', async (stroke, ack = (_result: Record<string, unknown>) => {}) => {
    ...
    socket.on('ink:erase', async ({ ids }, ack = (_result: Record<string, unknown>) => {}) => {
    ...
    socket.on('ink:clear', async (_payload, ack = (_result: Record<string, unknown>) => {}) => {
      await DesktopStroke.deleteMany({ desktopKey: 'shared-desktop' });
      io.to(room).emit('ink:cleared');
      ack({ ok: true });
    });
    socket.on('text:commit', async (annotation, ack = (_result: Record<string, unknown>) => {}) => {
    ...
    socket.on('text:update', async (annotation, ack = (_result: Record<string, unknown>) => {}) => {
    ...
    socket.on('text:delete', async ({ id }, ack = (_result: Record<string, unknown>) => {}) => {
    ...
    socket.on('window:commit', async ({ itemId, patch, revision }, ack = (_result: Record<string, unknown>) => {}) => {
