...
export function requireAuth(req, res, next) {
  ...
  try {
    const payload = jwt.verify(token, desktopJwtSecret());
    if (typeof payload === 'string') throw new Error('Invalid session payload');
    req.user = payload;
    return next();
  } catch {
...
export async function authenticateDesktopToken(token) {
  ...
  if (typeof payload === 'string' || payload.type !== 'desktop' || !payload.jti) throw new Error('Invalid desktop session');
